...
/**
 * Chatbot.jsx — chat logic unchanged: same chatAPI.send payload (message,
 * history, user_data with identical keys + gender mapping), same store
 * actions, same scroll-to-bottom behaviour, same markdown-lite rendering.
 * All visible strings come from the chat i18n namespace.
 */
const SUGGESTION_KEYS = ['calories', 'plateau', 'workoutPlan', 'postWorkout', 'breakPlateau']
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className={`max-w-[75%] ${isUser ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
        ...
        <span className="text-xs text-white/25 px-1">
          {isUser ? t('chat:you') : `${t('chat:coachLabel')}${msg.model ? ` · ${msg.model}` : ''}`}
        </span>
      </div>
    </motion.div>
  )
...
export default function Chatbot() {
  ...
  async function sendMessage(text = input) {
    ...
    try {
      ...
      addMessage({ role: 'assistant', content: t('chat:fallbackReply'), model: 'fallback' })
    ...
    <div className="flex flex-col h-[calc(100vh-6rem)] animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <h1 className="text-2xl font-bold font-display">{t('chat:title')} <span className="gradient-text">{t('chat:titleAccent')}</span></h1>
        ...
        <button onClick={clearChat} className="btn-ghost flex items-center gap-2 text-xs py-2 px-3">
          <RotateCcw size={13} /> {t('chat:newChat')}
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pb-4">
        {chatHistory.length === 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
            ...
            <h3 className="font-bold font-display text-lg mb-2">{t('chat:ready')}</h3>
            <p className="text-white/40 text-sm mb-6 max-w-sm mx-auto">
              {t('chat:readySubtitle')}
            </p>
            <div className="flex flex-wrap gap-2 justify-center">
              {SUGGESTION_KEYS.map(key => {
                const s = t(`chat:suggestions.${key}`)
                return (
                  <button key={key} onClick={() => sendMessage(s)} className="chip text-xs">
                    {s}
                  </button>
                )
              })}
